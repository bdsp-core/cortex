...
class PosteriorUpdateError(RuntimeError):
    """A posterior update failed without mutating the particle state."""
...
def update(state, k, s, y, s_sd=0.0):
    ...
    """
    if int(y) not in (0, 1):
        raise ValueError(f"binary response y must be 0 or 1, got {y!r}")
    if not np.isfinite(s) or not np.isfinite(s_sd) or float(s_sd) < 0.0:
        raise ValueError(f"invalid signal parameters s={s!r}, s_sd={s_sd!r}")

    old_w = np.asarray(state["w"])
    old_log_lik = np.asarray(state["log_lik"])
    old_w_sum = float(old_w.sum())
    if (not np.all(np.isfinite(old_w)) or np.any(old_w < 0.0)
            or not np.isfinite(old_w_sum) or old_w_sum <= 0.0):
        raise PosteriorUpdateError("pre-update particle weights are invalid")
    if not np.all(np.isfinite(old_log_lik)):
        raise PosteriorUpdateError("pre-update log likelihood is invalid")
    ...
    new_log_lik = old_log_lik + log_p_obs
    w = old_w * np.exp(log_p_obs)
    s_w = float(w.sum())
    if (not np.all(np.isfinite(log_p_obs))
            or not np.all(np.isfinite(new_log_lik))
            or not np.all(np.isfinite(w))
            or not np.isfinite(s_w) or s_w <= 0.0):
        raise PosteriorUpdateError(
            "posterior update has zero mass or non-finite values")

    # Commit only after every derived field has passed validation.
    state["log_lik"] = new_log_lik
    state["w"] = w / s_w
...
def resample_and_rejuvenate(state, rng, n_mh_steps=15, proposal_scale=0.5,
    ...
    distinct_ancestors = int(np.unique(idx).size)
    state["t"] = state["t"][idx]
    state["l"] = state["l"][idx]
    ...
    # Additive, behavior-preserving diagnostics used by the precision-policy
    # reliability guard. The scalar return value remains unchanged.
    state["_last_rejuvenation"] = {
        "q_index": int(q_index) if q_index is not None else -1,
        "acceptance_rate": float(accept_rate),
        "distinct_ancestors": distinct_ancestors,
        "distinct_ancestor_fraction": float(distinct_ancestors / N),
    }
    if diag_callback is not None:
...
def _expected_loss_vec(state, k, signals, signal_sds=None,
                       objective="total_var", ell_star=None):
    ...
    objective (discrimination_rd Lever A; default "total_var" ⇒ BYTE-IDENTICAL):
      "total_var"  — expected total posterior variance over (t_1..t_K, l_1..l_K)
                     [the shipped A-optimal objective; unchanged].
      "skill_var"  — drop the nuisance bias block: expected Σ_j Var(l_j) only
                     (the AD6 verdict + AUROC depend on ℓ alone).
      "cut_focused"— expected verdict-uncertainty Σ_j π_j(1−π_j) at the cut,
                     π_j = P(l_j > ell_star[j]); requires ell_star (len K).
    """
    K = state["t"].shape[1]
    t_k = state["t"][:, k]
    l_k = state["l"][:, k]
    ...
    elif objective == "skill_var":
        total_y1 = sum(var_vec(state["l"][:, kk], w_y1) for kk in range(K))
        total_y0 = sum(var_vec(state["l"][:, kk], w_y0) for kk in range(K))
    elif objective == "cut_focused":
        if ell_star is None:
            raise ValueError("objective='cut_focused' requires ell_star (len K)")
        es = np.asarray(ell_star, dtype=float)

        def _vunc(weights):
            tot = np.zeros(weights.shape[0])
            for kk in range(K):
                g = (state["l"][:, kk] > es[kk]).astype(float)
                pi = (weights * g[None, :]).sum(axis=1)
                tot = tot + pi * (1.0 - pi)
            return tot
        total_y1 = _vunc(w_y1)
        total_y0 = _vunc(w_y0)
    elif objective.startswith("weighted"):
        # min E[ Σ Var(ℓ_j) + w·Σ Var(t_j) ] — tighten BOTH skill and bias, with
        # bias down-weighted by w∈[0,1] ("weighted_<w>"; w=0⇒skill_var, w=1⇒total_var).
        # Does NOT move the cut. discrimination_rd Phase 6 (both-tightness + ℓ–t decoupling).
        wt = float(objective.split("_", 1)[1]) if "_" in objective else 1.0
        total_y1 = sum(var_vec(state["l"][:, kk], w_y1) for kk in range(K)) + \
                   wt * sum(var_vec(state["t"][:, kk], w_y1) for kk in range(K))
        total_y0 = sum(var_vec(state["l"][:, kk], w_y0) for kk in range(K)) + \
                   wt * sum(var_vec(state["t"][:, kk], w_y0) for kk in range(K))
    else:
...
def _coarse_to_fine_top_indices(loss_of, n, n_top, n_coarse,
                                m_bracket=None):
    """Deterministic coarse-to-fine approximation to the top-N indices.

    This extends the F4.1 campaign lever to trial-0 top-N randomisation.  The
    production default remains an exhaustive scan (``n_coarse=None``).  In
    development mode, refining at least the top ``n_top`` coarse basins makes
    the candidate set deliberately wider than the argmin-only F4.1 search.
    A separate drift test compares this result with the exhaustive top-N on
    the compact parity fixture before the frontier harness may use it.
    """
    n = int(n)
    n_top = min(max(1, int(n_top)), n)
    full = np.arange(n)
    if n_coarse is None or n <= int(n_coarse):
        losses = loss_of(full)
        return full[np.argsort(losses, kind="stable")[:n_top]]
    nc = int(n_coarse)
    brackets = max(n_top, 3) if m_bracket is None else int(m_bracket)
    coarse = np.unique(np.round(np.linspace(0, n - 1, nc)).astype(int))
    coarse_losses = loss_of(coarse)
    h = int(np.ceil(n / nc))
    order = np.argsort(coarse_losses, kind="stable")[:brackets]
    windows = [coarse]
    for j in order:
        center = int(coarse[int(j)])
        windows.append(np.arange(
            max(0, center - h), min(n - 1, center + h) + 1))
    candidates = np.unique(np.concatenate(windows))
    losses = loss_of(candidates)
    return candidates[np.argsort(losses, kind="stable")[:n_top]]


def _uncertainty_aware_coarse_to_fine_top_indices(
        loss_of, n, n_top, n_coarse, uncertainty):
    """F4.1 extension for banks with irregular per-item signal uncertainty.

    ``s_mean`` ordering alone is not smooth when neighbouring items have very
    different ``s_sd``. Each equal-width signal bin therefore contributes its
    centre and its minimum-uncertainty item to the coarse stage. Full local
    windows around the best coarse basins are then evaluated exactly. This is
    opt-in and leaves every shipped/default selection path unchanged.
    """
    n = int(n)
    n_top = min(max(1, int(n_top)), n)
    if n_coarse is None or n <= int(n_coarse):
    ...
    if uncertainty.shape != (n,) or np.any(~np.isfinite(uncertainty)):
        raise ValueError("uncertainty must be a finite vector aligned to candidates")
    bins = np.array_split(np.arange(n), int(n_coarse))
    representatives = []
    for indices in bins:
        if len(indices) == 0:
            continue
        representatives.append(int(indices[len(indices) // 2]))
        representatives.append(int(indices[np.argmin(uncertainty[indices])]))
    representatives = np.unique(representatives)
    representative_losses = loss_of(representatives)
    best = np.argsort(representative_losses, kind="stable")[:max(3, n_top)]
    h = int(np.ceil(n / int(n_coarse)))
    windows = [representatives]
    for position in best:
        center = int(representatives[int(position)])
        windows.append(np.arange(
            max(0, center - h), min(n - 1, center + h) + 1))
    candidates = np.unique(np.concatenate(windows))
    losses = loss_of(candidates)
...
def choose_item(state, bank_signals=None, active_domains=None,
                ...
                bank_segids=None, objective="total_var", ell_star=None,
    ...
    for k in domains:
        ...
        if uncertainty_aware_subsample:
            if sds is None:
                raise ValueError(
                    "uncertainty-aware subsampling requires bank_sds")
            idx = int(_uncertainty_aware_coarse_to_fine_top_indices(
                loss_of, len(sigs), 1, n_subsample, sds)[0])
        else:
            idx = _coarse_to_fine_argmin(
                loss_of, len(sigs), n_subsample)
        loss = float(_expected_loss_vec(
            state, k, sigs[idx:idx + 1],
            signal_sds=(sds[idx:idx + 1] if sds is not None else None),
            objective=objective, ell_star=ell_star)[0])
        if loss < best_loss:
            best_loss = loss
            best_k = k
