"""SMC sampler with Metropolis-Hastings rejuvenation,
...
Replaces the West kernel-jitter resampling with proper
...
Hierarchical prior on (t_1,...,t_K, l_1,...,l_K):
  ...
  empirical corpus_a cross-domain rater matrix
...
Response model includes a uniform lapse rate :
...
def _log_p_response(z, y):
    ...
    logsumexp to mix in the lambda lapse floor:
        P(y=1|z) = λ + (1 − 2λ)·Φ(z)
        P(y=0|z) = λ + (1 − 2λ)·Φ(−z)
...
def choose_item(state, bank_signals=None, active_domains=None,
    ...
    Per-domain arrays may differ in length.
...
def simulate_response(s, t_true, l_true, rng):
    ...
    F0.1: Eq. 2 symmetric lapse mixture (floor λ, ceiling 1 − λ).
    """
