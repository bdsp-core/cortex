"""Termination + verdict policies — the AD6 resolution.
...
the rest of the test (session controller, storage, results screen) treats
...
Three policies ship:
  ...
  AD6Policy          — the resolved rule (default for production).
                       ...
                       Session stops when all tasks are RESOLVED, the question cap is hit, or
                       the bank is exhausted.
                       This is the unanimous output of a expert independent panel.

  DeltaStopPolicy    — the legacy Mode-A AUROC half-width rule
                       (max_k HW < δ). Retained for the methodology /
                       Paper-1 path; not the live-test default. In fact we will remove this before
...
it. The default policy is built from
...
# cert_config lives in the repo-root config/ directory.
_REPO = Path(__file__).resolve().parents[1] / "config"
...
# (each one is independently OC-calibratable in the harness).
#
#
...
#                                    random-rater edge case observed where
#				     N_MIN=12 floor prematurely ended some domain testing
...
def load_ell_star(task_codes, config_path=None):
    """Load the Youden ℓ*_k for the tasks
    """
    import yaml
    path = Path(config_path) if config_path else (_REPO
                                                  / "cert_config_general.yaml")
    if not path.exists():
...
class DeltaStopPolicy(TerminationPolicy):
    """Legacy (will remove before production push) Mode-A rule: stop when max-k AUROC half-width < delta.
...
class AD6Policy(TerminationPolicy):
    ...
    def from_inputs(cls, inputs, **kwargs):
        """Construct from an EngineInputs: ℓ*_k from cert_config v13,
        Var_prior from the engine's fitted Corr_l diagonal."""
        ell_star = load_ell_star(inputs.task_codes)
...
def default_policy_for(inputs, *, delta_auroc=None,
    ...
    """Resolve the policy used by the test. Precedence:
