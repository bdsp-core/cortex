""" Test — adaptive-session controller.

Drives an adaptive certification session on the SMC + MCMC
particle-cloud engine (engine/core_mcmc.py). Two layers in one file:

  * ``Session`` — the pure, Qt-free adaptive loop. Built entirely
    ...
    ``policy.default_policy_for`` precedence ladder). Testable
    headless with any ``y_source`` callable.

  * ``EngineWorker`` + ``SessionController`` — the Qt wrapper.
    EngineWorker runs a Session inside a background QThread with a
...
from engine_inputs_k7 import build_k7_engine_inputs  # noqa: E402
from policy_k7 import default_policy_for_k7  # noqa: E402
from policy import PENDING  # noqa: E402 — v1.2.4: phase-aware active-domains

# Phase-9 K=7: K=7 wins by default. Module remains K-agnostic via
# inputs.task_codes; the build_k7 loader supplies 7 tasks (domain1 + 6 ).
# Back-compat aliases preserve the old names for callers that haven't
# migrated; the underlying loader is K=7 either way.
build_engine_inputs = build_k7_engine_inputs   # back-compat alias
...
# Session honours min(MAX_QUESTIONS_DEFAULT, bank_size) — the
# bank-size floor preserves audit/OC scripts that pass max_questions=None
# (==bank_size). Only the live test path (signal_bank_viewer.main) passes
# this constant explicitly.
MAX_QUESTIONS_DEFAULT = 500
# v1.3.6: raised 300 -> 500 for a paper-grade config. At
...
# 700-seg bank (100 /class) lifts clear-candidate per-domain resolution from
...
# many questions in a row on one  task the selector is forced to switch
# domains, to break up long single-domain runs (one rater hit 99 domain2 in a
# row, session e46cc793). Chosen from sim_v1_3_5/run_consec_sweep.py (300
# heterogeneous-rater sessions): cap=12 capped the worst  run from ~158 to
...
# sets this; Session's own default is None (off) for tests/sims/audit.
# v1.3.6: lowered 12 -> 5 (maintainer's call) for more variety and less exploitability
...
# Internal-test release (2026-06-04): keep asking questions PAST the point
...
# of the PASS/FAIL/REFER thresholds (the pilot showed they are likely too
...
# Session defaults it OFF so every test / sim / audit is byte-identical to
...
class SessionResult:
    ...
    # Extended data-collection mode (internal-test release). When the
...
class Session:
    """Pure, Qt-free  adaptive session over the SMC engine."""

    def _init_(self, inputs, *, session_id="adhoc",
        ...
        #   "variance"  — A-optimal total posterior variance (SHIPPED default;
        #                 byte-identical to pre-Step-3).
        #   "decision"  — decision-aligned: minimise expected Σ π_k(1−π_k) over
        #                 the still-PENDING tasks (targets verdict resolution).
        #                 Requires a policy exposing per-task cut-scores
        #                 (`ell_star`); falls back to "variance" otherwise.
        if selection_objective not in ("variance", "decision"):
            raise ValueError(
                f"selection_objective must be variance|decision, got "
                f"{selection_objective!r}")
    ...
    def _compute_active_domains(self, bank_signals):
        """K=7 phase-aware active-domains for the engine selector. The maintainer's
        v1.2.4 sectioning rule + Issue-4 empty-bank guard:

          Phase A (domain1-first sectioning): if K=7 AND domain1 (k=0) verdict
            is still PENDING AND the domain1 bank is non-empty, return
            [0] only — the engine asks domain1 questions until AD6 locks
            the domain1 verdict OR the domain1 bank exhausts.

          Phase B ( sectioning): once the domain1 phase has ended (verdict
            locked or pool empty), return all  task indices (1..6)
            with non-empty banks AND PENDING verdicts.

          K=6 (legacy) / non-AD6 policies: return all k with non-empty
            banks (no phase awareness needed —  banks are uniform).
        ...
        50 domain1 segs by ~trial 140 and crash on the 51st pick because
        domain1's bank_signals[0] was shape (0,)). Phase-aware
        ...
        # (ignore verdict locks + domain1-first sectioning) so the engine keeps
        ...
        if self._post_decision:
            ...
            # Phase A — K=7 domain1 sectioning (the domain1 block is deliberate; the
        ...
        # is the ONLY unresolved one, fall back to other  tasks with
        # non-empty banks (even already-resolved) so a long single-domain run
        # is broken up. k>=1 keeps the variety question a 6-way  (not a
        # domain1 yes/no). The forced question still updates the joint posterior.
    ...
    def _select(self, state, remaining, rng, trial_index):
        ...
        if self.selection == "adaptive":
            ...
            # v1.2.4: phase-aware active domains (domain1-first sectioning +
            ...
            if trial_index == 0 and self.first_item_topn > 1:
                ...
                # Step 3: decision-aligned objective when enabled + the policy
                # exposes cut-scores. decision_tasks = still-PENDING tasks (so
                # the objective scores resolution over every open verdict, not
                # just the domain1-first active set). ell_star=None ⇒ the engine's
                # default A-optimal selector (byte-identical).
                ell_star = None
                decision_tasks = None
                if (self.selection_objective == "decision"
                        and getattr(self.policy, "ell_star", None) is not None):
                    ell_star = self.policy.ell_star
                ...
                k, s, s_sd, seg_id = choose_item(
                    ...
                    ell_star=ell_star, decision_tasks=decision_tasks)
    ...
    def _pick_top_n(self, state, bank_signals, bank_sds, bank_segids, rng,
        ...
        v1.2.4: respects `active_domains` (the K=7 phase-A domain1-first
...
if _HAVE_QT:
    ...
    class EngineWorker(QObject):
        """Runs one Session inside a background QThread. The
        ...
        def _init_(self, session: Session, answer_queue: queue.Queue):
            super()._init_()
        ...
        def _y_source(self, k, seg_id, s):
            ...
            0-based 6-way  pick; the one-vs-rest reduction happens here
    ...
    class SessionController(QObject):
        ...
        def _init_(self, inputs, session_id, parent=None, **session_kw):
            super()._init_(parent)
            self.inputs = inputs
            self.session_id = str(session_id)
            self.session = Session(inputs, session_id=self.session_id,
                                         **session_kw)
        ...
        def submit_answer(self, raw_choice: int):
            """Hand the rater's raw 0-based 6-way  pick to the engine."""
...
if _name_ == "_main_":
    ...
    sess = Session(inp, session_id="smoke", n_particles=600)
