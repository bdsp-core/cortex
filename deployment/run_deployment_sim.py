...
def sample_candidate_theta(tier_cfg, r_ell, rng):
    ...
    Bias t_k drawn independently across tasks.
    """
    ...
    ts = mu_t + sd_t * rng.standard_normal(K)
    theta = np.zeros(DIM)
    for k in range(K):
...
def main(seed=0, out_dir=None):
    ...
    print(f"per-task ℓ* = {dict(zip(st.TASKS, np.round(ell_star, 3)))}", flush=True)
    ...
    for tier, tcfg in TIERS.items():
        ...
        for j in range(tcfg["n"]):
            theta = sample_candidate_theta(tcfg, r_ell, rng)
