...
def main():
    ...
    fsi = np.exp(-(ELL - 0.10))
    scenarios = [
        ("trainable", dict(weak_task=2, l_true=0.0, rule="soft",
                           use_mixture=True, budget=240), range(10)),
        ("near_cut", dict(weak_task=2, l_true=float(ELL[2] - 0.30), rule="soft",
                          use_mixture=True, budget=200), range(10)),
        ("static_below", dict(weak_task=2, l_true=0.15, rule="static",
