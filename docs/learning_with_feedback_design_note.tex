...
prediction-error rule. Let
\begin{equation}
  \delta_k \;\equiv\; y_k - y_k^{\star}
  \quad \in \quad \{-1, 0, +1\}
  \label{eq:delta}
\end{equation}
be the \textbf{outcome prediction error} on trial $k$, where
$y_k^{\star}$ is the ground-truth label revealed by the feedback
$\mathrm{fb}_k$. Three cases:
\begin{itemize}[leftmargin=*,itemsep=1pt]
  \item $\delta_k = +1$: false alarm. Learner said yes; correct
        answer was no. The criterion is too liberal — raise it.
  \item $\delta_k = -1$: miss. Learner said no; correct answer was
        yes. The criterion is too conservative — lower it.
  \item $\delta_k = 0$: correct. No update.
\end{itemize}
This gives
\begin{equation}
  \boxed{\;
  ...
  \label{eq:tdyn}
\end{equation}
i.e.\ Eq.~\eqref{eq:tdyn} with the explicit form $g_t(s_k, y_k,
\mathrm{fb}_k) = \delta_k$.
...
\begin{description}[leftmargin=2.2em,style=nextline,itemsep=1pt]
  \item[$\alpha_t \ge 0$.] Criterion learning rate, in units of the
    ...
    Typical magnitudes from the SDT-criterion-learning literature
    \citep{stuttgen2011trial} place $\alpha_t$ between $0.05$ and
    $0.2$.
  ...
  \item[$\delta_k$.] As in Eq.~\ref{eq:delta}: the only thing that
    drives $t$ is being wrong, and the direction of the move is set
    by the direction of the error.
\end{description}
