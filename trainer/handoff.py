"""The eval→trainer handoff artifact `TrainingSeed` (G1 port of trainer_rd
training_seed). Verbatim numerics port; bit-parity gated by
`tests/test_trainer_g1_handoff.py` against scratch golden vectors.

The certification session ends with a joint SMC particle cloud, AD6 verdicts, and
a question history; the trainer starts from exactly that, with variance inflation
(D1) and a per-task marginal view (D2). All latent parameters are stored in ENGINE
coordinates (θ, ℓ); convert with `trainer.conventions.engine_to_plan`.

`build_seed_from_state` takes a plain engine-state dict ("t", "l", "w", "history",
optional "history_s_sd"), so it is data-source-agnostic. The thin adapter mapping
the production `SessionResult` (session_controller) → that dict is added when the
G2 orchestrator wires the loop.
"""
...
SCHEMA_VERSION = 1
DEFAULT_INFLATE = 1.5          # D1 starting value
...
# ───────────── trainer-side per-trial record ─────────────

@dataclass
class TrialRecord:
    """One training (or eval) trial — the unit the Phase-3 dynamics fits read.
    rt_ms / answer_changes are NaN/-1 when the presentation layer cannot supply
    them; they must still be logged (F10)."""
    session_id: str
    trial_index: int
    task_k: int
    seg_id: int
...
def inflate_cloud(x, w, factor):
    ...
    Multiplies every weighted central moment's scale by exactly `factor` (weighted
    SD ×= factor; weighted mean unchanged)."""
...
@dataclass
class TrainingSeed:
    """Everything the trainer needs from a finished eval session. (N, K) clouds
    are the joint posterior; the per-task marginal for task k is
    (theta[:, k], ell[:, k], w) (D2), exposed via `task_cloud`."""
    ...
    def eval_seen_segids(self):
        """Seg-ids served during the eval (exposure-exclusion input)."""
...
def build_seed_from_state(state, *, session_id: str, ell_star,
    ...
    state: the engine state ("t", "l", "w", "history", optional "history_s_sd").
    ell_star: per-task cut-scores ℓ*_k (mastery targets, F14).
    verdicts/diagnostics: AD6 outputs. Optional for non-cert sessions.
    seg_ids / rt_ms / trial_epochs / post_decision: per-trial parallel lists;
    None ⇒ −1 / NaN / NaN / False placeholders (fields exist regardless)."""
...
# ───────────── learner ledger (D9 — cross-session data structure) ─────────────

class LearnerLedger:
    """Append-only sequence of session/seed references for ONE learner. The
    slow-timescale models (consolidated trait, forgetting kernel) are fitted from
    this. JSON on disk; entries are never mutated or removed, only appended."""
