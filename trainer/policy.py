"""Tier-2 mode-conditional policy + cross-task scheduler (G1 port of trainer_rd
trainer_policy). Verbatim numerics port — only the imports below repoint to the
ported `trainer/` modules; bit-parity gated by `tests/test_trainer_g1_policy.py`
against scratch golden vectors on synthetic candidates.
...
from trainer.conventions import SKILL_MODE_MULTIPLIER, engine_to_plan
from trainer.handoff import TrialRecord
...
def bias_correction_score(filt, cands):
    ...
    from trainer.reward import RewardWeights, _expected_reward
...
class TaskModePolicy:
    ...
    def __init__(self, task, ell_star, sigma_star, thresholds: ModeThresholds):
        ...
        # label-schedule randomization (docs/LABEL_SCHEDULE_PECR.md), attached
        # by TrainerPolicy when label_schedule="randomized"; all None (the
        # default) ⇒ every legacy deterministic device runs bit-identically.
        # _lsched: the per-task label walk (skill sides + bias wants +
        # cert-probe wants all consult it — the walk tracks only trials it
        # PROPOSED, flagged info["sched"]; retention/maintenance serving is
        # scoped out of the label-schedule surface); _flip: the cap-2
        # probe/corrective type walk. The mirror device under the
        # randomized schedule is STATIC paired-bin serving
        # (label_schedule.paired_bin_choice — history-free, so there is no
        # difficulty-echo channel; see PECR §5c for the two failed dynamic
        # predecessors). All walk/flip state advances at record()/
        # note_served — a step() whose choice is never record()ed must be
        # abandoned, not resumed.
        self._lsched = None
        self._flip = None
        self._bin_fallbacks = 0           # skill picks with no paired bin
    ...
    def select(self, mode, est, cands, retention, now, rng, filt=None):
        ...
        sig_hat, t_hat = est
        if mode == BIAS:
            # enforce running label balance: prefer the under-served label
            # (F6b) — legacy deterministic rule; under the randomized
            # schedule the want comes from the per-task label walk (whose
            # soft anti-imbalance tilt IS the F6b envelope, now with a
            # proven guessing bound) and the probe/corrective dual-control
            # flip from the cap-2 type walk.
            if self._lsched is not None:
                want = 1 if self._lsched.propose() > 0 else 0
                probe_now = self._flip.propose() > 0
            else:
                want = 0 if self._bias_label_balance > 0 else 1
                probe_now = self._bias_probe
            target = t_hat                              # near the criterion
            if filt is not None and not probe_now:
            ...
            was_probe = probe_now or filt is None
            if self._flip is None:
                self._bias_probe = not probe_now
            info = {"target": target, "want_label": want, "probe": was_probe}
            if self._lsched is not None:
                info["sched"] = True      # walk/flip proposed → commit at record
            return idx, info
        if mode == SKILL:
            # side: strict ±1 alternation (legacy) or the per-task label walk
            side = (self._lsched.propose() if self._lsched is not None
                    else self._skill_side)
            ...
            if ((self.th.progress_placement or self.th.rollout_placement)
                ...
                if self._lsched is not None:
                    # STATIC paired-bin mask (PECR §5c) — side-blind bin
                    # choice; no history-dependent magnitude coupling.
                    # Opt-in path: progress_placement × randomized schedule
                    # is combinable but awaits its own study (PECR §6.3).
                    from trainer.label_schedule import paired_bin_choice
                    m_mask = paired_bin_choice(
                        view.s_mean, view.y_star, want,
                        SKILL_MODE_MULTIPLIER * max(sig_hat, 1e-6),
                        s_sd=view.s_sd)
                    if m_mask is None:
                        self._bin_fallbacks += 1
                    elif (m_mask & (view.y_star == want)).any():
                        sub = np.where(m_mask)[0]
                        view = view.subset(sub)
                        pick = (pick[sub] if pick is not None
                                else sub)
                elif (self._last_skill_s is not None
                ...
                if pi - 2.0 * mcse >= 0.70:      # F70 finishing threshold
                    ...
                    from trainer.rollout import rollout_item_q
                ...
                if self._lsched is None:
                    self._skill_side *= -1
                info = {"target": float(cands.s_mean[idx]),
            ...
            if self._lsched is not None:
                # STATIC paired-bin serving (randomized schedule, PECR §5c):
                # side-blind bin choice + within-bin want-side argmax makes
                # the served magnitudes side-matched BY CONSTRUCTION — the
                # M10 midpoint invariant with no history-dependent channel
                from trainer.label_schedule import paired_bin_choice
                want = 1 if side > 0 else 0
                bmask = paired_bin_choice(
                    cands.s_mean, cands.y_star, want,
                    SKILL_MODE_MULTIPLIER * max(sig_place, 1e-6),
                    s_sd=cands.s_sd)
                view, pick = cands, None
                if bmask is None:
                    self._bin_fallbacks += 1
                else:
                    pick = np.where(bmask)[0]
                    view = cands.subset(bmask)
                # within the side-blind bin, the want-side pick keeps the
                # F20 precision-aware score (E[w]) — dropping it was the
                # round-3a defect (served s_sd-blurred overlap items ⇒
                # sharp-margin FG 0.60 → 0.80, reverted §5c)
                ew = expected_skill_weight(view.s_mean, view.s_sd,
                                           sig_place, 0.0, side=side,
                                           m=SKILL_MODE_MULTIPLIER,
                                           rho=self.th.rho)
                j = self._argmax_score(ew, view, want_label=want)
                idx = int(pick[j]) if pick is not None else j
                self._last_skill_s = float(cands.s_mean[idx])
                return idx, {"target": side * SKILL_MODE_MULTIPLIER
                             * sig_place, "want_label": want, "sched": True}
    ...
    def note_served(self, mode, y_star, s=None, info=None):
        info = info or {}
        if mode == BIAS:
            self._bias_label_balance += 1 if y_star == 1 else -1
        # randomized-schedule commits (single site, at record time): only
        # trials the schedule PROPOSED advance the walk/flip — info["sched"]
        # is set at every proposal site, so retention and other
        # non-scheduled serving never feed the state, while pool-exhaustion
        # deviations (served label ≠ proposal) self-correct because the
        # SERVED label is what commits. (The §5c mirror device is static —
        # no per-trial state to advance.)
        if self._lsched is not None and info.get("sched"):
            self._lsched.commit(1 if y_star == 1 else -1)
            if self._flip is not None and mode == BIAS and "probe" in info:
                self._flip.commit(1 if info["probe"] else -1)
...
class TrainerPolicy:
    ...
    def __init__(self, filters, ell_stars, sigma_stars, bank, *,
                 ...
                 maint_probes=0, stale_alpha=0.05, label_schedule=None,
                 schedule_params=None, schedule_seed=None):
        ...
        # None (default) ⇒ bit-identical legacy deterministic devices.
        # "randomized" ⇒ per-task capped soft-tilted label walks + cap-2
        # type flips + mirror-debt ledgers. The schedule stream is a
        # counter-based splitmix64 keyed (schedule_seed, stream) — it never
        # touches self.rng, so legacy numpy draws are unchanged either way.
        self.label_schedule = label_schedule
        if label_schedule is not None:
            if label_schedule != "randomized":
                raise ValueError(f"unknown label_schedule: {label_schedule!r}")
            from trainer.label_schedule import (FLIP_PARAMS, ScheduleParams,
                                                ScheduleRng, SideScheduler)
            sp = schedule_params or ScheduleParams()
            s0 = seed if schedule_seed is None else schedule_seed
            for k, mp in enumerate(self.mode_policies):
                mp._lsched = SideScheduler(sp, ScheduleRng(s0, stream_id=2 * k))
                mp._flip = SideScheduler(FLIP_PARAMS,
                                         ScheduleRng(s0, stream_id=2 * k + 1))
        ...
        if (len(cands) > 0
                and (maintenance                    # M27 F90 at-bar probe
                     or (self.probe_every is not None and mode != RETENTION
                         ...
                         == self.probe_every - 1))):
            sig_hat, t_hat = est
            mp = self.mode_policies[task]
            sched_prop = mp._lsched is not None
            if sched_prop:
                # probe wants ride the same per-task label walk the modes
                # use — the learner sees one label stream per task
                want = 1 if mp._lsched.propose() > 0 else 0
            else:
    ...
    def record(self, choice, y, *, session_id="train", rt_ms=np.nan,
        ...
        self.mode_policies[task].note_served(mode, y_star, s=s,
                                             info=choice["info"])
