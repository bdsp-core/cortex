...
(deployed floor-0.15 reference, threshold 5.213):

**Tripped at wrong-pick 15** (statistic 6.04). The calibration predicted a
median of 14 wrong-picks to trip under a uniform-distractor world; this real
session tripped at 15. Near-uniform wrong picks are exactly the world the
monitor exists to detect, and in production a trip replays the session
binary-reduced — so roughly 85% of this sitting would have been scored by the
binary engine, and the served result would have been the incumbent's.

## Fixed-sequence three-engine replay (same items, same answers)

| engine | mean 95% skill-interval width |
|---|---|
| binary (production incumbent) | 1.929 |
| static mixture floor015 (the failed attempt) | 1.580 |
